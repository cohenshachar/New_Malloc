...
private:
    typedef struct MallocMetadata{
        ...
        MallocMetadata* next;
        MallocMetadata* prev;
        MallocMetadata() : size(0),is_huge(false), is_free(false), next(nullptr), prev(nullptr){}
    }* mallocMetadata;
...
public:
    ...
    void* lalloc_block(size_t size, bool hugePageFlag = false);
...
bool BlockManager::can_join(mallocMetadata free_block, size_t size, size_t dest_size){
    if(dest_size > (BLOCKS_SET_SIZE*KB - sizeof(struct MallocMetadata)))
        return false;
...
bool BlockManager::join(mallocMetadata* free_block){
    if((*free_block)->size > (BLOCKS_SET_SIZE*KB/2)-sizeof(struct MallocMetadata))
        return false;
...
void* BlockManager::lalloc_block(size_t size, bool hugePageFlag){
    int flags = MAP_PRIVATE | MAP_ANONYMOUS;
    if(hugePageFlag)
        flags = flags|MAP_HUGETLB;
    mallocMetadata block = (mallocMetadata)mmap(nullptr, size+sizeof(struct MallocMetadata), PROT_READ | PROT_WRITE, flags, -1, 0);
    block->size = size;
    block->is_free = false;
    block->next = nullptr;
    block->prev = nullptr;
    block->is_huge = hugePageFlag;
    totBlocks++;
    totBytes += size;
...
void BlockManager::alloc_free(void* p){
    ...
    if(isLargeAlloc(m->size)){
        ...
        int pageSize = m->is_huge ? 2*KB*KB : sysconf(_SC_PAGESIZE);
        if(size_allocated%pageSize != 0)
            size_allocated += pageSize - size_allocated%pageSize;
        if(munmap((void*)m, size_allocated) == -1){
...
void* smalloc(size_t size){
    ...
    if(BlockManager::getInstance().isLargeAlloc(size))
        return BlockManager::getInstance().lalloc_block(size,size >= 4*KB*KB);
...
void* scalloc(size_t num, size_t size){
    ...
    void* res;
    if(BlockManager::getInstance().isLargeAlloc(size))
        res = BlockManager::getInstance().lalloc_block(size*num,size >= 2*KB*KB);
    else
        res = BlockManager::getInstance().alloc_block(size*num);
    if(!res)
        return nullptr;
...
void* srealloc(void* oldp, size_t size){
    ...
    if(BlockManager::getInstance().isLargeAlloc(size)){
        if(size == BlockManager::getInstance().get_size(oldp))
            return oldp;
        res = BlockManager::getInstance().lalloc_block(size,size >= 4*KB*KB);
    }else
